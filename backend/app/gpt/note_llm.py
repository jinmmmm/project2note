...
1. 整体风格：讲解细致、语言友好；安装/操作/避坑/学习顺序/联网补充的可执行细节全部写入「## 2. 结构化笔记」内的 **4–6 个**归纳后的 ### 主题（**严格最多 6 个，超出不合格**），禁止逐步骤或分阶段各建 ###，内容多时在同一 ### 内扩展。
2. 一级结构固定 4 节且不可改：视频基础信息 → 结构化笔记 → 工具与链接补充（Markdown 表格）→ 延伸知识点（前置基础+后续进阶各 2 条）。用户额外要求仅可调整框架内细节。
3. ### 标题须简洁专业（如「环境安装」「版本回退」），禁止照搬博主口语/比喻式标题（如「后悔药」「越用越懂你」）；比喻只放正文或 term-defs。
4. 每个 ### 下必须有正文；无内容则不建该节，合并到相邻主题，禁止空 ###。
5. 联网补充（强制执行）：凡提及「安装/下载 xxx」或口播提到未演示安装的工具（如 cc-switch），必须在同段补齐获取链接或安装命令、环境要求、启动方式；禁止只写「安装 xxx」无命令/链接。写入第 2 节 ### 正文；第 3 节表格作快查索引。
6. 术语：正文 **术语** 标记 + 文末 <!-- term-defs --> **5–10 条**通俗释义（通俗易懂，可自然类比，禁止写「比喻：」，不必每条都比喻）；第 2 节术语首次出现须 **包裹**。
7. 禁止：术语表章节、纯链接 bullet 附录、超过 7 个 ###、独立避坑/学习路径 ## 大节。
8. 第 2 节每个 ### 标题行末尾须附 `<!-- ts:mm:ss -->`（来自逐字稿，勿把 `[mm:ss]` 写进标题文字）。"""
        else:
            system += """
 当前生成【专业精简版】笔记，面向有基础的从业者：
1. 一级结构固定 4 节且不可改：视频基础信息 → 结构化笔记 → 补充最新版本 → 延伸知识点（前置基础+后续进阶各 2 条）。用户额外要求仅可调整框架内细节。
2. 结构化笔记内 3–5 个 ### 主题，清单式列结论/命令/参数及联网补充；禁止逐步骤拆 ###。
3. ### 标题须简洁专业，禁止博主口语/比喻式标题（如「后悔药」「越用越懂你」）；每个 ### 必须有正文，无内容则合并。
4. 联网补充（强制执行）：即使是专业版，也必须补充基础下载/安装/入门配置/新手指令；写入第 2 节相关 ### 正文，不新增「工具链接补充」大节。若同时存在高阶要点，再补版本差异、高阶参数、生态工具、生产建议。
5. 术语用 **术语** 标记；<!-- term-defs --> 写 **4–7 条**精确定义（无需比喻）。
6. 禁止：== == 标记、术语表、避坑/学习路径 ## 大节、超过 5 个 ###。
7. 第 2 节每个 ### 标题行末尾须附 `<!-- ts:mm:ss -->`（来自逐字稿，勿写进标题可见文字）。"""
        return system
    ...
    def polish_markdown(self, content: str, instruction: str = "", style: str = "beginner") -> str:
        mark_rule = (
            ...
            "结构化笔记内 ### 4–7 个（最多 7 个），### 标题简洁专业、禁止比喻式标题，"
            "删除无正文空 ###，禁止术语表章节。"
            if style == "beginner"
            else (
