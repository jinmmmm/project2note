...
3. **### 标题命名（小白版专有）**：标题用简洁专业的知识主题名（2–8 字），如「环境安装」「权限模式」「版本回退」；**禁止**照搬博主原话、口语小标题、比喻/梗式命名（如「后悔药」「越用越懂你」「让 AI 成为专家」）；博主比喻可写在正文或 term-defs，**不得出现在 ### 标题**；标题禁止引号、感叹号、emoji。
4. **禁止空 ###**：每个 ### 下须有至少 1 条有效正文（bullet/段落/代码块）；内容不足则合并到最相关 ###，不得留下无正文的标题。
5. **术语释义**：文末 `<!-- term-defs -->` 写 **5–10 条**；格式 `- 术语 — 通俗解释`（1–2 句，语言浅显；需要时可自然用「就像…」类比，**禁止**写「比喻：」前缀，不必每条都比喻）；第 2 节术语**仅首次出现**用 **术语** 包裹，供 ? 悬浮释义；仅标记读者可能不懂的词，勿滥用 ** **。
6. **第 3 节工具与链接补充**：必须用 Markdown 表格 `| 工具/资源 | 说明 | 链接/获取方式 |` 汇总快查；禁止仅用 bullet 链接列表代替表格。
7. 禁止单独输出：概述总结、分章节内容、新手避坑指南、学习路径、视频目录等 ## 大节。
"""

STYLE_PROFESSIONAL = """
## 风格：专业精简版
1. **结构化笔记（第 2 节）**：清单式输出结论、命令、参数及联网补充；**3–5 个 ### 主题**，合并冗余步骤，禁止逐步骤拆 ###。
2. **### 标题命名**：用简洁专业主题名（2–8 字），如「权限模式」「版本回退」「记忆配置」；**禁止**博主口语、比喻/梗式标题（如「后悔药」「越用越懂你」）；标题禁止引号、感叹号、emoji。
3. **禁止空 ###**：每个 ### 下须有有效正文；内容不足则合并到相邻主题，不得留空标题。
4. 术语标记：专业术语用 **术语** 标记；term-defs 写 **4–7 条**精确定义（无需比喻）；仅标记领域专有词。
5. **专业版也必须补齐基础可执行信息**：凡提到工具/软件/CLI，正文内要给出下载或安装方式、基础配置、首次使用指令；同时可保留版本差异、高阶用法、生态工具、优化建议。
6. **第 3 节补充最新版本**：版本适配、高阶用法、生态工具、优化建议（条目或短表格）；无拓展内容可写「暂无」。
7. 禁止：避坑指南、学习路径、逐步骤 ##/### 镜像视频目录、额外新增「工具链接补充」大节。
"""
...
- ### 标题用简洁专业主题名，**禁止**博主口语/比喻式小标题；每个 ### 必须有正文，无内容则合并；
...
4. **重写 ### 标题**：改为简洁专业主题名，去除博主口语、比喻、梗（如「后悔药」「越用越懂你」）；**保留** `<!-- ts:mm:ss -->` 注释。
...
3. 节点 label 保持 4-16 字关键词，禁止长段落、禁止口语废话。
