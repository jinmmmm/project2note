...
- **两版差异必须在既有 4 节内体现**：小白版写成「跟着走、学着懂」的教程；专业版写成「查结论、拿命令」的手册/速查表。
...
## 联网补充规则（专业版 · 强制执行）
专业版同样**必须补充**：基础下载、安装、入门配置、新手指令；但写法必须短、可检索，一级框架保持不变，不得新增「工具链接补充」大节。
**写法要求**：
- **第 2 节正文**：凡视频提到但未完整演示的工具、软件、插件、CLI、资源，必须在相关 ### 内写出获取方式 / 安装命令 / 环境要求 / 入门配置 / 首次启动或新手指令；用代码块、参数表、短 bullet 表达，禁止小白式长解释。
- **第 3 节「补充最新版本」**：强化专业专属价值，必须按 `### 版本兼容与升级`、`### 进阶与优化`、`### 已知问题与 Workaround` 三个三级小节输出，不使用表格。每个小节 **1–3 条**短 bullet；每条必须是 `- **关键词**：结论/建议`，关键词 2–8 字；禁止裸 bullet 分类（如单独一行「版本兼容」「高阶用法」），禁止空小节，禁止一行拼多个要点。可顺带标注「下载 / 安装 / 配置见第 2 节某主题」，但不能替代第 2 节正文。
- **已知问题与 Workaround**：用「问题 — 最短解决方案」格式；一句话点出坑和修复，不写手把手排查过程。
- 不需要像小白版那样单独做「工具链接补充」表格；基础信息和专业结论放在一起，保持手册 / 速查表感。
- 若同时存在高阶要点与基础操作，先给最短可执行命令，再补版本差异、进阶参数、生产建议。
- **目标**：默认读者具备领域知识，打开即可检索结论、复制命令、确认版本/Workaround，无需额外搜索。
"""
...
STYLE_BEGINNER = """
## 风格：小白学习版
1. **定位：教程感**：面向零基础用户，写成「跟着走、学着懂」的教程；先解释为什么，再给操作，再告诉怎么确认成功。
2. **第 2 节开场（强制）**：在「结构化笔记」开头先输出 4 组短 bullet：`适合人群`、`不适合人群`、`前置知识检查`、`学完能达成的目标`；每组 1–3 条，语言具体，不写空泛鸡汤。
3. **结构化笔记（第 2 节）是核心正文区**：安装步骤、命令、避坑、学习顺序、联网补充的可执行细节全部写在这里，用 `###` 主题拆分。
4. **### 主题归纳规则（强制）**：**严格限制 4–6 个 ###**，按「主题/模块」聚合归纳，**超过 6 个视为不合格**；禁止为「第一步/第二步/第 X 阶段/各功能点」各建 ###；多个相关步骤/功能必须合并到同一 ### 下的有序/无序列表；内容多时加密正文而非拆分新 ###。
5. **### 标题命名（小白版专有）**：标题用简洁专业的知识主题名；**字数强制要求：中文字 + 英文单词合计 ≤ 6 个**（如 Harness、Engineering 各算 1 个，"什么是 Harness Engineering" = 3+2=5 个但含禁止前缀词，不合格）；**禁止**以「什么是/如何/为什么/如何理解」等疑问修饰词开头；正确示例：「Harness 概念」「环境安装」「权限模式」；错误示例：「什么是 Harness Engineering」「如何理解四大理论」；**禁止**照搬博主原话、口语小标题、比喻/梗式命名（如「后悔药」「越用越懂你」）；博主比喻可写在正文或 term-defs，**不得出现在 ### 标题**；标题禁止引号、感叹号、emoji。
6. **讲解模板（强制差异化）**：概念类主题按 `是什么 / 为什么重要 / 怎么用` 讲；安装、配置、操作类主题按 `操作目的 / 具体步骤 / 成功验证 / 常见卡点` 写。常见卡点写成新手可跟做的修复路径，不单独开「新手避坑指南」大节。
7. **正文 bullet 格式（思维导图兼容，强制）**：每条 bullet 正文的第一个名词/动词短语（2–8 字）必须用 `**粗体**` 包裹作为关键词，其后可接冒号或空格加说明；**禁止**以完整陈述句开头（如「视频提到将手把手搭建…」「目前适合实现…」）；**禁止**把整句话加粗；**禁止** `**Step 1：...**` 序号式粗体，应写 `**关键步骤名**：具体说明`。
8. **禁止空 ###**：每个 ### 下须有至少 1 条有效正文（bullet/段落/代码块）；内容不足则合并到最相关 ###，不得留下无正文的标题。
9. **术语释义**：文末 `<!-- term-defs -->` 写 **5–10 条**；格式 `- 术语 — 通俗解释`（1–2 句，语言浅显；需要时可自然用「就像…」类比，**禁止**写「比喻：」前缀，不必每条都比喻）；第 2 节术语**仅首次出现**用 **术语** 包裹，供 ? 悬浮释义；仅标记读者可能不懂的词，勿滥用 ** **。
10. **第 3 节工具与链接补充**：必须用 Markdown 表格 `| 工具/资源 | 用途 | 环境要求 | 安装/获取方式 | 启动/自检方式 |` 汇总快查；禁止仅用 bullet 链接列表代替表格。完整命令、配置、验证仍必须写在第 2 节正文。
11. **第 4 节延伸知识点**：写成学习路径指引，告诉新手先补什么基础、学完后往哪里进阶；可附入门资料关键词，但仍保持固定 4 条 bullet 模板。
12. 禁止单独输出：概述总结、分章节内容、新手避坑指南、学习路径、视频目录等 ## 大节。
"""

STYLE_PROFESSIONAL = """
## 风格：专业精简版
1. **定位：手册 / 速查表感**：面向有基础的从业者，写成「查结论、拿命令」的检索式笔记；默认读者知道背景，不做小白式铺垫。
2. **第 2 节开场（强制）**：先写 `核心结论`，**100 字以内**，纯干货结论，无铺垫；随后写 `核心要点速览`，3–5 条 bullet，只列结论、边界、适用场景。
3. **结构化笔记（第 2 节）**：清单式输出结论、命令、参数、配置、适用边界及联网补充；**3–5 个 ### 主题**，可用「指令集 / 参数配置 / 集成流程 / 风险边界」等专业标题；合并冗余步骤，禁止逐步骤拆 ###。
4. **禁止小白式长解释**：专业版正文不得展开 `是什么 / 为什么 / 操作目的 / 成功验证` 模板；除风险提示外，不写手把手教学过程。
5. **### 标题命名**：用简洁专业主题名；**字数强制要求：中文字 + 英文单词合计 ≤ 6 个**（如 Harness、Engineering 各算 1 个）；**禁止**以「什么是/如何/为什么/如何理解」等疑问修饰词开头；正确示例：「权限模式」「版本回退」「Harness 概念」；**禁止**博主口语、比喻/梗式标题；标题禁止引号、感叹号、emoji。
6. **正文 bullet 格式（思维导图兼容，强制）**：每条 bullet 正文的第一个名词/动词短语（2–8 字）必须用 `**粗体**` 包裹作为关键词，其后可接冒号或空格加说明；**禁止**以完整陈述句开头；**禁止**把整句话加粗；**禁止** `**Step 1：...**` 序号式粗体，应写 `**关键步骤名**：具体说明`。
7. **命令与参数**：命令优先用可复制代码块；参数、配置、版本差异优先用短表格，文字越少越好。
8. **列表换行强制**：每个条目必须独立占一行并以 `- ` 开头；禁止在同一 bullet 内用 ` - `、`-工具名`、`-问题` 连续拼接多个条目。
9. **禁止空 ###**：每个 ### 下须有有效正文；内容不足则合并到相邻主题，不得留空标题。
10. 术语标记：专业术语用 **术语** 标记；term-defs 写 **4–7 条**精确定义（无需比喻）；仅标记领域专有词。
11. **专业版也必须补齐基础可执行信息**：凡提到工具/软件/CLI，正文内要给出下载或安装方式、基础配置、首次使用指令；同时保留版本差异、高阶用法、生态工具、优化建议。
12. **第 3 节补充最新版本**：必须按 `### 版本兼容与升级`、`### 进阶与优化`、`### 已知问题与 Workaround` 三个三级小节输出，不使用表格。每个小节 **1–3 条**短 bullet；每条必须是 `- **关键词**：结论/建议`，关键词 2–8 字；禁止裸 bullet 分类、空小节、长段 bullet 和多个要点拼同一行。
13. **第 4 节延伸知识点**：偏官方文档、源码、release notes、相关技术资料；少给入门教程。
14. 禁止：避坑指南、学习路径、逐步骤 ##/### 镜像视频目录、额外新增「工具链接补充」大节。
"""
...
## 2. 结构化笔记
- 开头必须先写「课前须知」，用短 bullet 分组列出：
  - **适合人群**：哪些零基础/初学者适合看。
  - **不适合人群**：哪些人可能不需要这篇教程。
  - **前置知识检查**：开始前要具备或准备什么。
  - **学完能达成的目标**：看完能完成哪些具体动作。
- 正文用 **4–6 个 ### 主题小节** 展开（安装/操作/避坑/练习顺序/联网补充的可执行细节写入对应 ###，不要另开 ##）：
  ...
  - 概念类主题按 `是什么 / 为什么重要 / 怎么用` 讲清楚；安装/配置/操作类主题按 `操作目的 / 具体步骤 / 成功验证 / 常见卡点` 写，常见卡点给一步步解决方法；
  - 步骤用有序/无序列表写在同一 ### 下；
...
- **必须使用 Markdown 表格**，表头固定为：
  `| 工具/资源 | 用途 | 环境要求 | 安装/获取方式 | 启动/自检方式 |`
- 每行一个工具/资源，作保姆级快查索引；无链接写「见第 2 节对应主题」或「官方站点搜索 xxx」。
- **禁止**在本节仅用 `- [名称](url)` bullet 列表代替表格；**禁止**仅写表格而第 2 节缺少完整命令、步骤、验证方式。

## 4. 延伸知识点
（见 EXTENSION_SECTION 固定格式；小白版必须写成下一步学习路径：前置基础=先补什么基础，后续进阶=学完后往哪里深入，可附适合新手搜索的入门资料关键词）

整体要求：4 个一级 ## 固定；第 2 节 4–6 个归纳后的 ###（严格不超过 6 个）；新手在第 2 节正文即可理解原因、复制操作、验证是否成功。
"""
...
## 2. 结构化笔记
- 开头必须写 `核心结论`：100 字以内，纯干货结论，无铺垫。
- 随后写 `核心要点速览`：3–5 条 bullet，只列关键结论、边界、适用场景。
- **3–5 个 ### 主题小节**：清单式列结论、命令、参数、配置及联网补充；可用「指令集 / 参数配置 / 集成流程 / 风险边界」等检索式主题；合并相近内容，禁止逐步骤拆 ###。
- **每个 ### 标题行末尾附 `<!-- ts:mm:ss -->`**（来自逐字稿，勿写进标题可见文字）；
- ### 标题用简洁专业主题名，中文字 + 英文单词合计 ≤ 6 个，**禁止**以「什么是/如何/为什么」开头，**禁止**博主口语/比喻式小标题；每个 ### 必须有正文，无内容则合并；
- 专业术语用 **术语** 标记；代码/命令单独成块；参数/配置/版本差异优先用短表格。
- **禁止小白式长解释**：不得套用「是什么 / 为什么 / 操作目的 / 成功验证」教学模板；除风险提示外，不写手把手过程。
- **专业版同样必须补齐基础执行信息**：视频提到但未完整展开的工具/软件/CLI，要在对应 ### 内写出下载或安装方式、环境要求、基础配置、首次使用命令；不得只保留高阶结论。

## 3. 补充最新版本
- 必须按以下 3 个 `###` 三级小节输出，顺序固定，不使用表格：
  - `### 版本兼容与升级`
  - `### 进阶与优化`
  - `### 已知问题与 Workaround`
- 每个三级小节 **1–3 条**短 bullet；每条必须是 `- **关键词**：结论/建议`，关键词 2–8 字。
- 禁止裸 bullet 分类（如单独一行「版本兼容」「高阶用法」「性能/生产建议」）；禁止空小节；禁止长段 bullet；禁止把多个要点拼在同一行。
- 不新增小白版那种「工具与链接补充」大节；基础下载/安装/配置信息仍以内文方式写在第 2 节，本节只做专业拓展与索引。

## 4. 延伸知识点
（见 EXTENSION_SECTION 固定格式；专业版应偏官方文档、源码、release notes、相关技术资料，少给入门教程）

整体要求：极致精简，4 个一级标题清晰，### 不超过 5 个；打开像手册 / 速查表。
"""

MERGE_PROMPT_BEGINNER = """
你将收到同一视频的多个 Markdown 笔记片段，请合并为一份完整笔记：
1. 保持 4 个一级 ## 结构：视频基础信息 → 结构化笔记 → 工具与链接补充（表格）→ 延伸知识点。
2. 保留小白版「教程感」：第 2 节开头必须有适合人群 / 不适合人群 / 前置知识检查 / 学完能达成的目标；概念类保留是什么/为什么/怎么用，操作类保留操作目的/具体步骤/成功验证/常见卡点，禁止压缩成专业版短清单。
3. 结构化笔记内 ### 归纳合并为 **4–6 个主题**（严格最多 6 个，超过不合格），禁止 7+ 个小节；联网补充的可执行细节（地址/命令/配置/验证方式）保留在第 2 节对应 ### 正文。
4. **删除无正文的空 ###**；将零散内容并入最相关主题，不保留空标题。
5. **重写 ### 标题**：改为简洁专业主题名，中文字 + 英文单词合计 ≤ 6 个，禁止「什么是/如何/为什么」开头，去除博主口语、比喻、梗（如「后悔药」「越用越懂你」）；**保留** `<!-- ts:mm:ss -->` 注释。
6. 保留 **术语** 标记、完整步骤与 term-defs 隐藏块；合并去重术语释义。
7. 第 3 节工具与链接补充必须是表格，优先使用 `| 工具/资源 | 用途 | 环境要求 | 安装/获取方式 | 启动/自检方式 |`，删除纯 bullet 链接附录。
8. 延伸知识点写成下一步学习路径：### 前置基础 + ### 后续进阶 各 2 条 bullet。
9. 禁止术语表可见章节；输出纯 Markdown。
"""

MERGE_PROMPT_PROFESSIONAL = """
你将收到同一视频的多个 Markdown 笔记片段，请合并为一份完整笔记：
1. 保持 4 个一级 ## 结构：基础信息 → 结构化笔记 → 补充最新版本 → 延伸知识点。
2. 保留专业版「手册 / 速查表感」：第 2 节开头必须有 `核心结论`（100 字以内）和 `核心要点速览`；正文只保留结论、命令、参数、配置、适用边界，禁止展开成小白式长解释。
3. 结构化笔记内 ### 合并为 **3–5 个主题**，去重精简命令与参数；**与工具相关的下载链接、安装命令、基础配置、首次使用指令必须保留在第 2 节正文**，不得只保留高阶摘要。
4. **删除无正文的空 ###**；将零散内容并入最相关主题。
5. **重写 ### 标题**：改为简洁专业主题名，去除博主口语、比喻、梗；**保留** `<!-- ts:mm:ss -->` 注释。
6. 术语统一 **术语** 标记；保留 term-defs 隐藏块。
7. 第 3 节必须保留 `### 版本兼容与升级`、`### 进阶与优化`、`### 已知问题与 Workaround` 三个三级小节；若第 2 节已有安装步骤，本节可引用但不得替代。每个小节必须有内容，禁止空小节。
8. 延伸知识点保留 ### 前置基础 + ### 后续进阶 各 2 条 bullet，偏官方文档、源码、release notes、相关技术资料。
9. 禁止术语表可见章节；输出纯 Markdown。
"""
...
def normalize_professional_summary_label(markdown: str) -> str:
    """兼容旧/漏遵循输出：TL;DR 统一显示为中文核心结论，并补齐冒号。"""
    if not markdown:
        return markdown
    markdown = re.sub(r"\*\*\s*TL\s*;?\s*DR\s*\*\*\s*[：:]?", "**核心结论**：", markdown, flags=re.I)
    markdown = re.sub(r"(?m)^\s*TL\s*;?\s*DR\s*[：:]?", "核心结论：", markdown, flags=re.I)
    markdown = re.sub(r"\*\*\s*核心结论\s*\*\*\s*(?![：:])", "**核心结论**：", markdown)
    markdown = re.sub(r"(?m)^\s*核心结论\s*(?![：:])", "核心结论：", markdown)
    return markdown
...
_SCREENSHOT_SKIP_HEADING_KEYWORDS = (
    "核心结论",
    "核心要点",
    "TL;DR",
    "课前须知",
...
def _should_skip_screenshot_heading(title: str) -> bool:
    return any(keyword in title for keyword in _SCREENSHOT_SKIP_HEADING_KEYWORDS)
    ...
    *,
    ...
    for line in lines:
        ...
        if in_section2 and heading and len(heading.group(1)) >= 3:
            ...
            if ts_match:
                section_title = _TS_COMMENT_RE.sub("", heading.group(2)).strip()
                if _should_skip_screenshot_heading(section_title):
                    pending_seconds = None
                    pending_section_title = ""
                    pending_body_preview = ""
                    pending_filename = None
...
def postprocess_note_markdown(
    ...
    *,
    ...
    markdown = normalize_professional_summary_label(markdown)
...
3. 节点 label 保持精炼关键词，禁止超过 16 字，禁止长段落、口语废话。
...
9. 仅基于「结构化笔记」内的 ### 主题小节内容生成导图；**禁止**把开场信息作为分支节点写入：小白版的「课前须知 / 适合人群 / 不适合人群 / 前置知识检查 / 学完能达成的目标」，专业版的「核心结论 / 核心要点速览」；**也禁止**把第4节「延伸知识点 / 前置基础 / 后续进阶」作为分支——这些是阅读引导与学习路径，不是第2节 ### 知识结构，必须忽略。
10. 输出格式：仅返回标准 JSON，无额外解释、无 markdown 代码块包裹。
